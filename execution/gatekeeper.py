...
class Gatekeeper:
            ...
            if snapshot:
                ...
                if meta_result:
                    ...
                    if not meta_result.allow_trading:
                        ...
                        logger.warning("MetaDecisionBrain blocked signal for %s: %s", symbol, meta_result.reason)
                        self.blocked_signals_count += 1
                        self._update_state()
                        # Сохраняем trace ПОСЛЕ принятия решения
                        self._save_decision_trace(symbol, snapshot, trace_entries, final_decision="BLOCK")
            ...
            if snapshot:
                ...
                if sizing_result:
                    ...
                    if not sizing_result.position_allowed:
                        ...
                        logger.info("[SIZER] Trade blocked for %s: %s", symbol, sizing_result.reason)
                        self.blocked_signals_count += 1
                        self._update_state()
                        # Сохраняем trace ПОСЛЕ принятия решения
                        self._save_decision_trace(symbol, snapshot, trace_entries, final_decision="BLOCK")
    ...
    def _check_risk_core(
        ...
        try:
            ...
            # Серия убытков — в событиях (11.09.2026): убытки, закрытые в пределах окна от
            # самого нового убытка события, — одна ставка. Раньше каждая сделка считалась
            # отдельно, по порядку ОТКРЫТИЯ (id) и вперемешку с бумажными: пять SHORT по
            # альтам, выбитые одним отскоком за 14 минут, дали паузу 60 минут.
            ...
            try:
                ...
                consecutive_losses, last_loss_timestamp = loss_streak(
                    database.get_recent_closes(on_exchange=sends_real_orders()))
