...
# Сам JSON — ~450 символов, но модель тратит до ~1000 токенов на скрытое рассуждение
# (в ответ оно не попадает): при 1500 первые мнения заняли 1271–1370. Платится только
# использованное, поэтому запас не стоит денег, а обрыв стоит — оценка без мнения.
