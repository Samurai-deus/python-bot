...
class MeanReversionStrategy(BaseStrategy):
    ...
    def evaluate(
        ...
        # Условие 1: State C or D (rejection/reversal) на 15m
        from core.market_state import MarketState
        state_15m = states.get("15m")
        if state_15m not in (MarketState.C, MarketState.D):
            return None

        # Условие 2: RSI экстремальный (H-4: relaxed for crypto)
        ...
        if rsi_val < 35 and bb_position in ("BELOW_LOWER", "LOWER"):
            ...
            # H-5: structural stop with min distance
            recent_low = min(float(c[3]) for c in candles_15m[-5:])
            stop = min(recent_low - atr_val * 0.1, entry - atr_val * 0.5)

        elif rsi_val > 65 and bb_position in ("ABOVE_UPPER", "UPPER"):
            ...
            stop = max(recent_high + atr_val * 0.1, entry + atr_val * 0.5)
        else:
        ...
        # Scale confidence by RSI extremity
        if side == "LONG":
            confidence = 0.50 + (35 - rsi_val) * 0.01
        else:
            confidence = 0.50 + (rsi_val - 65) * 0.01
