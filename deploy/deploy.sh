...
step_smoke() {
  ...
  # Исполнитель И13 (market-bot-carry): пульс пишет каждый часовой цикл; первый — сразу после старта.
  i=0
  cry=""
  while [ $i -lt 18 ]; do
    cry=$(docker inspect -f '{{.State.Health.Status}}' market-bot-carry 2>/dev/null || echo missing)
    [ "$cry" = healthy ] && break
    i=$((i + 1)); sleep 10
  done
  if [ "$cry" = healthy ]; then echo "  ok  сбор фандинга (И13): healthy"; else echo "  ОШИБКА сбор фандинга (И13): $cry"; fail=1; fi

  # Исполнитель И14 (market-bot-portfolio): пульс каждый часовой цикл; первый — сразу после старта.
  i=0
  pf=""
  while [ $i -lt 18 ]; do
    pf=$(docker inspect -f '{{.State.Health.Status}}' market-bot-portfolio 2>/dev/null || echo missing)
    [ "$pf" = healthy ] && break
    i=$((i + 1)); sleep 10
  done
...
step_mode() {
  ...
  case "$target" in
    demo)
      ...
      pairs="PAPER_TRADING=false DRY_RUN=false LIVE_TRADING=false BYBIT_TESTNET=false BYBIT_DEMO=true REAL_CAPITAL_CAP_USDT=1000 RISK_PERCENT=1"
