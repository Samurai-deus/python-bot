...
подключены к API только на чтение. Посмотреть экраны без Telegram: `cd miniapp && npm run dev:mock` (данные из фикстур, см. `miniapp/README.md`). Цепочка «ветка → PR → CI → слияние» для скриптов — `deploy/pr_chain.sh`.
