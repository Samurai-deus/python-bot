...
## 🔬 Исследовательская программа (с 11.09.2026)

Сигнальная торговля бота выключена по правилу И14 (`SIGNAL_TRADING_ENABLED=false`); пока она выключена, цикл анализа
короткий — свечи и проверка резких движений, без «мозгов», Decision Core и генерации сигналов, интервал на проде
`BOT_INTERVAL=1800` (30 минут; его же читает проверка здоровья контейнера); включите флаг — вернётся полный цикл. Демо-счета отданы исследовательской
программе — [`docs/TRADER_PLAN.md`](docs/TRADER_PLAN.md) (правила записываются до прогона), разведка — [`docs/MARKET_ATLAS.md`](docs/MARKET_ATLAS.md),
открытый техдолг — [`docs/TECH_DEBT.md`](docs/TECH_DEBT.md). Контейнеры на проде:

| Контейнер | Пакет | Что делает |
|---|---|---|
| `market-bot-portfolio` | `portfolio/` | И14 — портфель тренд + моментум + продолжение на демо-счёте бота; календарь уведомлений (`portfolio/calendar.py`) |
...
Базис срочных фьючерсов (И20) — `py -m backtest.basis download|run|holdout --db <путь>`; шорт листингов (И21) — `py -m backtest.listings run --db <кэш широкой вселенной>`.
...
подключены к API только на чтение. Посмотреть экраны без Telegram: `cd miniapp && npm run dev:mock` (данные из фикстур, см. `miniapp/README.md`). Цепочка «ветка → PR → CI → слияние» для скриптов — `deploy/pr_chain.sh` (`ship_branch <ветка> <коммит> <PR> <заголовок> <файлы…>` — полный прогон, коммит, PR, CI, слияние).
Внешнее движение средств на счёте исполнителя (демо-средства, перевод) — `python -m portfolio.adjust --db <база> --amount <USDT> --reason "…"`, иначе результат исполнителя врёт на эту сумму.
...
Мутационная проба (тест, который не краснеет на поломке, ничего не охраняет): `py -m tools.mutate <файл> <старое> <новое>`,
прогнать нужные тесты, вернуть файл `git checkout -- <файл>`. Вхождение должно быть ровно одно, иначе инструмент откажет.
Мутант ставится на саму ветку поведения, а не на вспомогательную функцию рядом: 23.09.2026 дважды тест проверял
помощника (`_signal_trading_enabled`) или сверял величину с самой собой — мутант в ветке выживал.
