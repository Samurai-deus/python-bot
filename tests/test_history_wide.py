...
def test_universe_keeps_only_old_crypto_usdt_perps_and_fills_instruments(tmp_path):
    ...
    assert symbols == ["BTCUSDT", "OLDUSDT"], "NEW — моложе года, TSLA — не крипта, ETHUSDC — не USDT"
    assert totals["instruments"] == 2 and totals["candles"] > 0
    assert conn.execute("SELECT turnover24h FROM instruments WHERE symbol = 'BTCUSDT'").fetchone()[0] == 9e9
    assert conn.execute("SELECT COUNT(DISTINCT symbol) FROM candles WHERE interval = '4h'").fetchone()[0] == 2
